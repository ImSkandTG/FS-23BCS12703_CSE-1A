...
function NewTransaction() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await UserService.add_transaction(user.email, data.category, data.description, data.amount, data.date);

      if (response.data.status === "SUCCESS") {
        navigate("/user/transactions", { state: { text: response.data.response } });
      } else {
        toast.error(response.data.response || "Unexpected error");
      }
    } catch (error) {
      toast.error(error.response?.data?.response || "Failed to add transaction: Try again later!");
    }
  ...
  return (
    <Container activeNavId={2}>
      ...
      {!isFetching && categories.length !== 0 && (
        <>
          <TransactionTypeSelectWrapper transactionTypes={transactionTypes} setTransactionType={setTransactionType} activeTransactionType={activeTransactionType} />
